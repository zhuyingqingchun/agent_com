...
class Agent(BaseAgent):
    ...
    相比基础版本，增加了：
    1. 任务级 phase / milestone 状态机；
    2. 候选区域化点击与局部裁剪图；
    3. app-level playbook memory 写回与检索；
    4. 更强的中断/弹窗/卡住页面恢复策略；
    5. prompt 中补充更多国内常用生活类 App 先验。
    """
...
1. 当前原始截图
2. 当前带网格截图（全局定位）
3. TOP_BAR / CENTER_PANEL / MID_LIST / BOTTOM_BAR / LEFT_PANEL / RIGHT_PANEL 六个局部裁剪图
4. 若有，则附带上一帧原始截图与上一帧网格截图

候选区域说明（可在 JSON 中额外返回 candidate_region，便于你更稳地指示点击区域）：
- TOP_BAR: 顶部搜索栏、标题栏、右上角操作区
- CENTER_PANEL: 中央主面板
...
动作空间只有：
1. CLICK: {{"action":"CLICK","parameters":{{"point":[x,y]}},"candidate_region":"MID_LIST"}}
...
""".strip()
    ...
    def _parse_with_regex(self, text: str) -> Tuple[str, Dict[str, Any]]:
        ...
        if action == ACTION_CLICK:
            region_match = re.search(
                r"(TOP_BAR|TOP_LEFT|TOP_RIGHT|CENTER_PANEL|MID_LIST|LEFT_PANEL|RIGHT_PANEL|BOTTOM_BAR|BOTTOM_LEFT|BOTTOM_RIGHT)",
                text.upper(),
            )
    ...
    def _post_process_action(
        ...
        if action == ACTION_COMPLETE:
            if input_data.step_count <= 2 or phase in {"launch", "home", "search_entry", "search_input", "submit_search"}:
                fallback_action, fallback_params = self._fallback_action(input_data, page_stuck=page_stuck)
        ...
        if action == ACTION_CLICK:
            ...
            point = self._snap_point_to_region(point, region)
    ...
    def _fallback_action(self, input_data: AgentInput, page_stuck: bool = False) -> Tuple[str, Dict[str, Any]]:
        ...
        if phase in {"home", "search_entry"}:
            return ACTION_CLICK, {"point": self._region_center("TOP_BAR")}
    ...
    def _recovery_action(self, input_data: AgentInput) -> Tuple[str, Dict[str, Any]]:
        ...
        if last_action == ACTION_TYPE:
            ...
            return ACTION_CLICK, {"point": self._region_center("TOP_RIGHT")}
